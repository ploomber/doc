...
def predict_input(model, single_input):
    ...
    input_df[encoded_cols] = model["encoder"].transform(
        input_df[categorical_cols].values
    )
...
def predict():
    ...
    return render_template(
        "index.html",
        prediction_text="The predicted weekly sales is "
        "$ {}".format(round(predicted_price, 2)),
    )
