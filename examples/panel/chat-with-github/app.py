...
class IndexLoader(pn.viewable.Viewer):
    ...
    owner: str = param.String(
        default="", doc="The repository owner. For example 'holoviz'"
    )
    repo: str = param.String(
        default="", doc="The repository name. For example 'panel'"
    )
    branch: str = param.String(
        default="", doc="The repository branch. For example, 'main'"
    )
    ignore_directories: str = param.String(
        default="",
        label="Folders to ignore",
        doc="A comma separated list of folders to exclude. For example 'examples,src'",
    )
    ignore_file_extensions: str = param.String(
        default="",
        label="File Extensions to ignore",
        doc="A comma separated list of file extensions to exclude. For example '.py,.txt'",
    )
    ...
    def __init__(self) -> None:
        ...
        self._layout = pn.Column(
            ...
            self.param.repo,
            self.param.branch,
            self.param.ignore_directories,
            self.param.ignore_file_extensions,
    ...
    async def _download_docs(self):
        ...
        ignore_directories = _split_and_clean(self.ignore_directories)
        ignore_file_extensions = _split_and_clean(self.ignore_file_extensions)
        ...
        return loader.load_data(branch=self.branch)
...
def create_chat_ui():
    ...
    pn.state.location.sync(
        ...
        parameters={
            ...
            "repo": "repo",
            "branch": "branch",
            "ignore_directories": "folders",
            "ignore_file_extensions": "file_extensions",
        },
    )
